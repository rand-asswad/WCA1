...
Base.eltype(m::STFT) = eltype(data(m))
Base.size(m::STFT, args...) = size(data(m), args...)
Base.getindex(m::STFT, args...) = getindex(data(m), args...)
Base.extrema(m::STFT) = extrema(data(m))
...
function istft(X::STFT)
    x = istft(data(X), or_length(X), width(X), noverlap(X);
...
@recipe function plot_spectrogram(S::STFT; mode=:spectrogram, yscale=:identity, axislabels=true)
    seriestype := :heatmap
